...
def process_bits():
	...
	code = psk.decode(cur_bits[:-len(sigil)])
...
#                diff -= 0.3
		if diff < 0:
                    ...
                    continue
		print "Mic %f" %micTime
		print "WiFi %f" %wifiTime
		print "Diff %f" %diff
		distance = diff*3400
		bus.write_byte(DEVICE_ADDRESS, int(distance))
                lock.acquire()
		hasWifi = False
		hasMic = False		
                lock.release()
...
processes = [process_frames, process_points, process_bits, wifi_connect, localize]
threads = []
